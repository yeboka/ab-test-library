...
export const variantAssigner = {
  assign(user: { id: string; email: string }, experimentKey: string, splits: Record<string, number>): string {
    const buckets = getNormalizedSplitsCached(experimentKey, splits)
    const cfg = getHashingConfig()
    const r = hashToBucketWithSalt(user, experimentKey, { salt: cfg.salt, version: cfg.version })
